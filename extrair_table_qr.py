...
def detect_qr_code(img: np.ndarray) -> Optional[str]:
    ...
    # Try to detect QR code in the top-left region (first 30% of width and height)
    h, w = gray.shape
    qr_region = gray[0:int(h*0.3), 0:int(w*0.3)]
    
    # Try multiple preprocessing techniques
    preprocessing_methods = [
        ("Original", qr_region),
        ("Adaptive Threshold", cv2.adaptiveThreshold(qr_region, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2)),
        ("Binary Threshold", cv2.threshold(qr_region, 127, 255, cv2.THRESH_BINARY)[1]),
        ("Otsu Threshold", cv2.threshold(qr_region, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)[1]),
    ]
    ...
    if qr_codes:
        ...
        print(f"[QR] Detected QR code using {successful_method}: {qr_data}")
    ...
    print("[QR] No QR code detected in top-left region")
    return None


def parse_qr_data(qr_data: str) -> Tuple[List[str], List[str]]:
    """
    Parse QR code data to extract question numbers and student names.
    
    Expected format:
    "question1;question2;question3.student1;student2;student3"
    
    Questions are separated by ';' and end with '.'
    Student names follow after '.' and are separated by ';'
    
    Args:
        qr_data: Raw QR code string
        
    Returns:
        Tuple of (question_headers, student_names)
    """
    try:
        # Split by '.' to separate questions from students
        parts = qr_data.split('.', 1)
        
        if len(parts) != 2:
            print(f"[QR Parse] Invalid format: expected 'questions.students', got: {qr_data}")
            return [], []
...
def build_final_table(img: np.ndarray, qr_data: Optional[str] = None):
    ...
    if qr_data:
        qr_questions, qr_students = parse_qr_data(qr_data)
        if qr_questions and qr_students:
            use_qr_mode = True
            print(f"[QR Mode] Using QR code data: {len(qr_questions)} questions, {len(qr_students)} students")
        else:
    ...
    if use_qr_mode:
        headers = get_qr_headers(img, col_intervals, header_row, qr_questions)
    else:
    ...
    # Extract student names using QR or OCR
    if use_qr_mode:
        raw_names = get_qr_names(img, name_col, candidate_student_rows, qr_students)
    else:
        raw_names = ocr_names(img, name_col, candidate_student_rows)

    student_rows, student_names = [], []
    
    # In QR mode, use all names from QR data without filtering
    if use_qr_mode:
        # Use the first N rows matching the number of QR students
        num_students = len(qr_students)
        student_rows = candidate_student_rows[:num_students]
        student_names = qr_students[:num_students]
        print(f"[QR Mode] Using {len(student_names)} students from QR code")
    else:
...
def main(qr_data: Optional[str] = None):
    ...
    # Use provided qr_data, or let build_final_table auto-detect (passing None)
    # build_final_table will try to detect QR, and if that fails, will use QR_DATA as fallback
    qr_input = qr_data if qr_data is not None else None
    df, df_conf, df_density, meta = build_final_table(pre, qr_data=qr_input)
