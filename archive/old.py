...
def preprocess_document(img: np.ndarray) -> np.ndarray:
    ...
    # Detectar contorno do documento e aplicar transformação de perspectiva
...
def identify_header_and_student_rows(img, row_intervals):
    ...
    The header row is the SHORTEST row among the first 2-3 rows.
    This is because the header only contains question numbers, while other rows contain
    QR codes, titles, or student names which require more vertical space.
    """
    ...
    student_rows = []
    for i, iv in enumerate(row_intervals[header_row_idx + 1:], start=header_row_idx + 1):
        h = iv[1] - iv[0]
